...
export function Sidebar({ onClose }: SidebarProps) {
  ...
  const navItems = [
    {
      label: t("sidebar.group_personal"),
      href: "/profile",
      icon: User,
      roles: ["admin", "employee", "supervisor", "ceo"],
      subItems: [
        { label: "ข้อมูลส่วนตัว (Profile)", href: "/profile", icon: User },
    ...
    {
      label: t("sidebar.group_directory"),
      href: "/directory",
      icon: Headset,
      roles: ["admin", "ceo"],
      subItems: [
        { label: t("sidebar.directory"), href: "/directory", icon: Contact },
        { label: t("sidebar.helpdesk"), href: "/helpdesk", icon: Headset },
        { label: t("sidebar.knowledge"), href: "/knowledge", icon: BookText },
      ]
    },
    {
    ...
    {
      label: t("sidebar.group_executive"),
      href: "/ceo",
      icon: FileBarChart,
      roles: ["admin", "ceo"],
      subItems: [
  ...
  useState(() => {
    // We can auto-initialize open state for the current active menu
    const activeGroup = navItems.find(item => {
      const isTeacherOk = !(item as any).requireTeacher || role === 'outsource' || role === 'admin' || profile?.is_teacher === true;
      if (!item.roles.includes(role.toLowerCase().trim()) || !isTeacherOk) return false;
      ...
      return item.subItems?.some(sub => {
        ...
        if (tabParam) {
          return pathname === subPath && searchParams.get('tab') === tabParam;
        }
    ...
    if (activeGroup) {
      setOpenSubmenus([activeGroup.label]);
    }
  });
    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar py-4">
        {filteredItems.map((item) => {
          ...
          // Parent active state check (returns true if any of the allowed subItems are active)
          const isParentActive = hasSubItems 
            ? item.subItems?.some((sub: any) => {
                // Filter subItems by user roles
                if (sub.roles && !sub.roles.includes(role.toLowerCase().trim())) return false;
          ...
          return (
            <div key={item.label} className="space-y-1">
                ...
                >
                  <div className="flex items-center gap-3">
                    <item.icon className={cn(
                      "h-5 w-5",
                      isParentActive ? "text-blue-400 animate-pulse" : "text-slate-500 group-hover:text-slate-300"
                    )} />
              ...
              {/* Render Sub Items */}
              {hasSubItems && isOpen && (
                <div className="ml-4 pl-4 border-l border-slate-800/80 space-y-1 animate-in slide-in-from-top-2 duration-200">
                  {item.subItems
                    ?.filter((sub: any) => {
                      if (!sub.roles) return true
                      return sub.roles.includes(role.toLowerCase().trim())
                    })
                    ?.map((sub) => {
                      ...
                      const isActive = pathname === subPath && (!tabParam || searchParams.get('tab') === tabParam)
                      ...
                      return (
                        ...
                        >
                          ...
                          {sub.href === "/approvals" && pendingCount > 0 && (
                            <Badge className="bg-red-500 text-white border-0 h-5 min-w-5 flex items-center justify-center rounded-full text-[10px] p-0 px-1 ml-auto">
                              {pendingCount}
                            </Badge>
                          )}
                        </Link>
